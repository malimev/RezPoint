...
const businesses = [
  ...
  { id: 2, name: "Q BAR", type: "Bar", location: "Gazimağusa", icon: "🍸" },
  { id: 3, name: "Sushinn Restaurant", type: "Restaurant", location: "Gazimağusa", icon: "🍣" },
];
const businessAccounts = [
  { email: "sky@rezpoint.com", password: "1234", businessId: 1 },
  { email: "qbar@rezpoint.com", password: "1234", businessId: 2 },
  { email: "sushinn@rezpoint.com", password: "1234", businessId: 3 },
];
...
function App() {
  ...
  const [availableTimes, setAvailableTimes] = useState(["18:00", "19:00", "20:30"]);
...
function getDistributionList(field) {
  ...
  return Object.entries(map)
    .map(([label, count]) => ({
      ...
      percent:
        accepted.length > 0
          ? Math.round((count / accepted.length) * 100)
          : 0,
    }))
    .sort((a, b) => b.count - a.count);
}
  ...
  function sendReservation() {
    ...
    if (reservation.phone === "") return setError("Please enter your phone number.");
    if (reservation.date === "") return setError("Please select a date.");
    if (reservation.time === "") return setError("Please select a time.");
    if (reservation.guests === "") return setError("Please enter number of guests.");
...
function closeDayReservations() {
  ...
  "Enter security code to close the day:"
);
        ...
        <section className="hero">
          <div className="hero-text">
            ...
            <p className="description">
              Customers choose a business, select a date and time, then create a reservation in seconds.
            </p>
...
>
  Create Reservation
          ...
          <div className="hero-card">
            <h3>How it works</h3>
            <div className="card-row"><span>1</span><strong>Login or create account</strong></div>
            <div className="card-row"><span>2</span><strong>Choose business</strong></div>
            <div className="card-row"><span>3</span><strong>Send request</strong></div>
          </div>
        </section>
      )}
  ...
  .map((business) => (
      ...
      {page === "reservation" && selectedBusiness && loggedCustomer && (
        <section className="reservation-section">
          <button className="back-btn" onClick={() => setPage("businesses")}>← Back</button>

          <div className="reservation-box">
            <h1>{selectedBusiness.name}</h1>
            <p className="description">
              Create your reservation request as <strong>{loggedCustomer.name}</strong>.
            </p>
            ...
            <form className="reservation-form">
              ...
              <p>Selected Date: <strong>{formatDate(reservation.date)}</strong></p>
              ...
              <div className="time-slots">
                {getAvailableDates().map((date) => (
                  <button
                    ...
                    onClick={() => setReservation({ ...reservation, date: date.fullDate })}
                  >
                    {date.display}
                  </button>
                ))}
              ...
              <p>Selected Time: <strong>{reservation.time || "Choose time"}</strong></p>
      ...
      {page === "summary" && selectedBusiness && loggedCustomer && (
        <section className="reservation-section">
          <div className="reservation-box">
            ...
            <div className="card-row"><span>Business</span><strong>{selectedBusiness.name}</strong></div>
            <div className="card-row"><span>Name</span><strong>{loggedCustomer.name}</strong></div>
            <div className="card-row"><span>Email</span><strong>{loggedCustomer.email}</strong></div>
            <div className="card-row"><span>Phone</span><strong>{reservation.phone}</strong></div>
            <div className="card-row"><span>Date</span><strong>{formatDate(reservation.date)}</strong></div>
            <div className="card-row"><span>Time</span><strong>{reservation.time}</strong></div>
            <div className="card-row"><span>Guests</span><strong>{reservation.guests}</strong></div>
      ...
      {page === "customerAuth" && (
        <section className="reservation-section">
          <div className="reservation-box">
            <h1>{customerMode === "login" ? "Customer Login" : "Create Account"}</h1>
            ...
            <div style={{ display: "flex", gap: "10px", marginBottom: "20px" }}>
              <button
                className={customerMode === "login" ? "selected-time" : "time-btn"}
                type="button"
              ...
              <button
                className={customerMode === "register" ? "selected-time" : "time-btn"}
                type="button"
            ...
            <form className="reservation-form">
              ...
              {customerAuthError && <p className="error-message">{customerAuthError}</p>}
                  ...
                  if (customerMode === "register") {
                    ...
                    const foundCustomer = registeredCustomers.find(
                      (customer) =>
                        ...
                        customer.password === customerForm.password
                    );
      ...
      {page === "customerVerify" && (
        <section className="reservation-section">
          <div className="reservation-box">
            ...
            <p className="description">
              We sent a verification code to <strong>{customerForm.email}</strong>.
            </p>
...
>
  ...
  <button
  className={customerTab === "notifications" ? "active-tab" : ""}
...
>
  ...
  <div className="reservation-box" style={{ marginTop: "20px" }}>
...
{customerTab === "profile" && (
  ...
  <p className="description">
    Your Safe Score increases when you attend reservations and decreases when you miss them.
  </p>
</div>
<p className="description">
  Your Safe Score reflects your reservation reliability. A higher score improves your chances of having future reservations accepted.
</p>

    <p className="description">
      These fields are optional. You can complete your profile anytime.
    </p>
...
<div className="time-slots">
  ...
  customerProfile.gender === "Male"
    ...
    onClick={() =>
      setCustomerProfile({ ...customerProfile, gender: "Male" })
    }
  >
    Male
  </button>
  ...
  <button
    type="button"
    className={customerProfile.gender === "Female" ? "profile-option selected-time" : "profile-option time-btn"}
    onClick={() =>
      setCustomerProfile({ ...customerProfile, gender: "Female" })
    }
  >
    Female
  </button>
  ...
  <button
    type="button"
    className={customerProfile.gender === "Prefer not to say" ? "profile-option selected-time" : "profile-option time-btn"}
    onClick={() =>
      setCustomerProfile({
        ...customerProfile,
...
<h3 style={{ marginTop: "20px" }}>Birth Date</h3>
      <input
        ...
        onChange={(e) =>
          setCustomerProfile({ ...customerProfile, birthDate: e.target.value })
        }
      />
      ...
      <input
        ...
        onChange={(e) =>
          setCustomerProfile({ ...customerProfile, job: e.target.value })
        }
      />
  ...
  <span>{rez.status}</span>
      ...
      <p className="description">
        No {customerTab} reservations found.
              ...
              <>
                <p className="description">Please login to see your reservations.</p>
                <button className="primary-btn" onClick={() => setPage("customerAuth")}>
                  Go to Login
                </button>
              </>
            )} 
    ...
    >
      ...
      {page === "businessPanel" && (
        <section className="business-panel-section">
          <div className="business-panel-header">
            <div>
              <h1>{loggedBusiness ? loggedBusiness.name : "Business Dashboard"}</h1>
              <p className="description">Manage your reservations and business settings.</p>
            </div>
...
>
  ...
  <button className={panelTab === "settings" ? "active-tab" : ""} onClick={() => setPanelTab("settings")}>
    Availability
  </button>

  <button className={panelTab === "profile" ? "active-tab" : ""} onClick={() => setPanelTab("profile")}>
    Business Profile
  </button>

  <button className={panelTab === "insights" ? "active-tab" : ""} onClick={() => setPanelTab("insights")}>
    Müşterini Tanı
  </button>
</div>
          ...
          <div className="panel-content">
            {panelTab === "incoming" && (
              <div className="reservation-box">
                ...
                {reservations.filter((rez) => rez.status === "pending").length > 0 ? (
                  reservations.filter(
  (rez) =>
  (rez.status === "pending" || rez.status === "cancelled") &&
  loggedBusiness &&
  rez.businessId === loggedBusiness.id
)
                    .map((rez) => (
      ...
      Reject
            ...
            {panelTab === "accepted" && (
              <div className="reservation-box">
                ...
                <p className="description">Select a date to view accepted reservations.</p>
  ...
  Close Day
            ...
            {panelTab === "rejected" && (
              <div className="reservation-box">
                ...
                <p className="description">Rejected reservation requests will appear here.</p>

                {reservations.filter((rez) => rez.status === "rejected").length > 0 ? (
                  reservations.filter(
                               (rez) =>
                                rez.status === "rejected" &&
                                loggedBusiness &&
...
{panelTab === "noShow" && (
      ...
      <p className="description">No no-show reservations yet.</p>
            ...
            {panelTab === "settings" && (
              <div className="reservation-box">
                ...
                <p className="description">Edit available days and time slots.</p>

                <h3>Reservation Mode</h3>

                <div className="time-slots">
                  <button
                    className={availabilityMode === "everyday" ? "selected-time" : "time-btn"}
                  ...
                  <button
                    className={availabilityMode === "selected" ? "selected-time" : "time-btn"}
                  ...
                  <>
                    ...
                    <div className="time-slots">
                      {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"].map((day) => (
                        <button
                          key={day}
                          className={availableDays.includes(day) ? "selected-time" : "time-btn"}
                          onClick={() => {
                ...
                {savedMessage && (
                  <p style={{ marginTop: "15px", color: "#86efac", fontWeight: "bold" }}>
                    {savedMessage}
                  </p>
                )}
            ...
            {panelTab === "insights" && (
              <div className="reservation-box">
                ...
                <p className="description">
                  Demo insights are shown for presentation. Real analytics will appear after enough customer data is collected.
                </p>
                ...
                <div className="panel-tabs" style={{ marginTop: "20px" }}>
                  ...
                  <button
                    className={customerInsightTab === "gender" ? "active-tab" : ""}
                  ...
                  <button
                    className={customerInsightTab === "firstTimers" ? "active-tab" : ""}
                  ...
                  <button
                    className={customerInsightTab === "topCustomers" ? "active-tab" : ""}
                  ...
                  <button
                    className={customerInsightTab === "smoking" ? "active-tab" : ""}
                  ...
                  <button
                    className={customerInsightTab === "busyDays" ? "active-tab" : ""}
                  ...
                  <button
                    className={customerInsightTab === "busyHours" ? "active-tab" : ""}
  ...
  <div style={{ marginTop: "24px" }}>
    ...
    {getDistributionList("gender").length > 0 ? (
      ...
      <p className="description">
        Cinsiyet dağılımı için kabul edilmiş rezervasyon gerekiyor.
      </p>
    )}
  </div>
)}
  ...
  <div style={{ marginTop: "24px" }}>
    ...
    {getCustomerFrequencyList().filter((customer) => customer.count === 1).length > 0 ? (
      getCustomerFrequencyList()
        .filter((customer) => customer.count === 1)
        .map((customer) => (
          <div className="accepted-list-item" key={customer.email}>
  ...
  <div style={{ marginTop: "24px" }}>
    ...
    {getDistributionList("smoking").length > 0 ? (
      ...
      <p className="description">
        Sigara dağılımı için kabul edilmiş rezervasyon gerekiyor.
      </p>
    )}
  </div>
)}
  ...
  <div style={{ marginTop: "24px" }}>
    ...
    {getBusyDaysList().length > 0 ? (
      ...
      <p className="description">
        Yoğun gün verisi için kabul edilmiş rezervasyon gerekiyor.
      </p>
    )}
  </div>
)}
  ...
  <div style={{ marginTop: "24px" }}>
    ...
    {getBusyHoursList().length > 0 ? (
      ...
      <p className="description">
        Yoğun saat verisi için kabul edilmiş rezervasyon gerekiyor.
      </p>
    )}
  </div>
)}
  ...
  {selectedReservation.code}
            ...
            <div className="card-row">
  <span>Gender</span>
  <strong>{selectedReservation.customerProfile?.gender || "Not provided"}</strong>
</div>

<div className="card-row">
  <span>Birth Date</span>
  <strong>{selectedReservation.customerProfile?.birthDate || "Not provided"}</strong>
</div>

<div className="card-row">
  <span>Job</span>
  <strong>{selectedReservation.customerProfile?.job || "Not provided"}</strong>
</div>

<div className="card-row">
  <span>Smoking</span>
  <strong>{selectedReservation.customerProfile?.smoking || "Not provided"}</strong>
</div>
            <div className="card-row">
              <span>Note</span>
